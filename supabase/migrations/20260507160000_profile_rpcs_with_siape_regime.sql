-- ============================================================
-- get_my_profile e list_profiles passam a expor siape e regime
-- ============================================================
--
-- AuthContext e painel /admin precisam dos novos campos para
-- exibir/editar.
...
AS $$
  SELECT p.id, p.display_name, p.email, p.team_id, p.role, p.is_active, p.siape, p.regime
...
BEGIN
  ...
  IF v_role IN ('admin_global', 'manager_cgris') THEN
    RETURN QUERY
    SELECT p.id, p.display_name, p.email, p.team_id, p.role, p.is_active, p.created_at, p.siape, p.regime
